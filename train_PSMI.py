...
from dataset.PSMI import create_dataset
from model import BERTTransformer, PSMIClassifier
...
#NOTE: the accuracy should be the balanced accuracy
#train_accuracy = BalancedSparseCategoricalAccuracy(name='train_accuracy')

#Note: no balanced accuracy when using weighted loss
...
def main(learning_rate, num_layers, num_heads, d_ff, d_model, dropout_rate, batch_size, keep_checkpoints, pretrain_checkpoint_dir,
    ...
    def train_step(inp, tar):
        ...
        interface = (504708+22923)/(2*22923) = 11.5
        non-interface = (504708+22923)/(2*504708) = 0.52
        ...
        loss_weights = tf.where(tf.math.not_equal(tar, 2), loss_weights, [22.02])
        #loss_weights = tf.where(tf.math.not_equal(tar, 1), loss_weights, [0.52])
    ...
    def accuracy_step(inp, tar):
        ...
        #loss_weights = tf.where(tf.math.not_equal(tar, 2), loss_weights, [22])
        #loss_weights = tf.where(tf.math.not_equal(tar, 1), loss_weights, [0.52])
