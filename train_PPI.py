...
from dataset.PPI import create_dataset
from model import BERTTransformer, PPIClassifier
...
def main(learning_rate, num_layers, num_heads, d_ff, d_model, dropout_rate, batch_size, keep_checkpoints, pretrain_checkpoint_dir,
    ...
    def train_step(inp, tar):
        ...
        interface = (906774 + 121020)/(2*121020) = 4.25
        non-interface = (906774 + 121020)/(2*906774) = 0.57
        ...
        #class imbalance weighted loss
        # loss_weights = tf.where(tf.math.not_equal(tar, 2), loss_weights, [4.25])
        # loss_weights = tf.where(tf.math.not_equal(tar, 1), loss_weights, [0.57])

        #class imbalance weighted loss
        loss_weights = tf.where(tf.math.not_equal(tar, 2), loss_weights, [7.49])
    ...
    def accuracy_step(inp, tar):
        ...
        #shapes
        #tar: [32 511]
        #predictions: [32 511 3]
        #loss_weights: [32 511]
        #So, every sequence in batch different line. Predictions have 3 output nodes. weights same size as tar.
