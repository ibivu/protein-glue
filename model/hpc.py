...
class HPCClassifier(tf.keras.Model):
    def __init__(self, dff, rate=0.1):
        super(HPCClassifier, self).__init__()

        self.nonlinear_layer = tf.keras.layers.Dense(dff, activation=tfa.activations.gelu)  # (batch_size, seq_len, dff)
        self.dropout = tf.keras.layers.Dropout(rate)
        self.classify_layer = tf.keras.layers.Dense(3) #2 classes (part of hydrophobic patch or not)
