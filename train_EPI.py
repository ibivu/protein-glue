...
from dataset.EPI import create_dataset
from model import BERTTransformer, EPIClassifier
...
#train_accuracy = BalancedSparseCategoricalAccuracy(name='train_accuracy')
...
POSITIONAL_ENCODING_MAX_LENGTH = 1200
...
def main(learning_rate, num_layers, num_heads, d_ff, d_model, dropout_rate, batch_size, keep_checkpoints, pretrain_checkpoint_dir,
    ...
    def train_step(inp, tar):
        ...
        """
        Manier 1:
        10X weight loss of 2
        loss_weights = tf.where(tf.math.not_equal(tar, 2), loss_weights, [10])

        Manier 2:
        How to deal with class imbalance:
        wj = n_samples/(n_classes * n_samples,j)
        interface = (41938 + 4503)/(2*4503) = 46441/ .. = 5.16
        non-interface = (41938 + 4503)/(2*41938) = 0.55

        Manier 3:
        Aantal keer dat het meer voorkomt.
        41938/4503 = 9.31
        ...
        #class imbalance weighted loss
        # loss_weights = tf.where(tf.math.not_equal(tar, 2), loss_weights, [5.16])
        # loss_weights = tf.where(tf.math.not_equal(tar, 1), loss_weights, [0.55])

        #class imbalance weighted loss
        loss_weights = tf.where(tf.math.not_equal(tar, 2), loss_weights, [9.31])
    ...
    def accuracy_step(inp, tar):
        ...
        if_prob_1D = tf.reshape(interface_prediction,[-1])
        ...
        return tar, predictions, loss_weights, tar_1D, predictions_max_1D, if_prob_1D
    ...
    for epoch in range(num_epochs):
        ...
        for (inp, tar) in ds:
            ...
            if batch % LOGGING_EVERY_STEPS == 0:
                ...
                # print ('Steps {} (Epoch {} Batch {}) Seqs/sec {:.1f} Accuracy {:.4f} Precision {:.4f} Recall {:.4f} AUC {:.4f}'.format(
                #     cur_step, epoch + 1, batch, seqs_per_sec, train_accuracy.result(),precision, recall,  auc_score))
