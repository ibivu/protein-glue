...
from dataset.HPR import create_dataset
from model import BERTTransformer, HPRClassifier
...
def main(learning_rate, num_layers, num_heads, d_ff, d_model, dropout_rate, batch_size, keep_checkpoints, pretrain_checkpoint_dir,
    ...
    def performance_step(inp, tar):
        ...
        #NOTE for regression accuracy not interesting. Look at pearson correlation coefficient
        # targets =  tf.reshape(tar, [tf.shape(tar)[0]*tf.shape(tar)[1]])
        # pred = tf.reshape(predictions, [tf.shape(predictions)[0]*tf.shape(predictions)[1]])

        #pearson = np.corrcoef(pred.numpy(), targets.numpy())[0][1]
        #return pearson

        #gaat fout omdat predictions niet meer 3 dimensies heeft.
        #indices = tf.squeeze(tf.where(tf.math.equal(loss_weights, 0)), 1)
    ...
    for epoch in range(num_epochs):
        ...
        for (inp, tar) in ds:
            ...
            if batch % LOGGING_EVERY_STEPS == 0:
                ...
                print ('Steps {} (Epoch {} Batch {}) Seqs/sec {:.1f} mae {:.4f} Loss {:.4f}'.format(
                    cur_step, epoch + 1, batch, seqs_per_sec, train_mae.result(), loss_train))
                ...
                loss_train = train_step(inp, tar)

                if writer:
                    with writer.as_default():
                        tf.summary.scalar('Loss (mae)', loss_train, cur_step)
